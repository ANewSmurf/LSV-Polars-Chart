...
$(function() {
	...
	Number.prototype.fix = function(n, pad) {
		...
		if (pad === true)
			return this.toFixed((n !== undefined) ? n : 5);
		else
			return Number.parseFloat(this.toFixed((n !== undefined) ? n : 5));
	};
...
var PolarsApp = (function(){
	...
	var _acceptedURLParams	= ["race_id", "twa", "tws", "light", "reach", "heavy", "foil", "hull", "utm_source", "nbdigits"];
  const tolerance = 1.014;
	...
	var _current_nbdigits	= 2;		// Display precision and smallest TWS step
	...
	function normalizeDigits(value, fallback) {
		...
		if (!Number.isFinite(parsed)) parsed = Number.parseInt(fallback, 10);
		if (!Number.isFinite(parsed)) parsed = _defaultSettings.nbdigits;
		return Math.max(0, Math.min(5, parsed));
	}
	function requestedDigits(urlParams, fallback) {
		if (urlParams["utm_source"] === "RouteMarins") return 5;
		return normalizeDigits(
			urlParams["nbdigits"] !== undefined ? urlParams["nbdigits"] : fallback,
			_defaultSettings.nbdigits
		);
	}
	...
	function buildWindSpeedButtons() {
		...
		var digits = normalizeDigits(_current_nbdigits, _defaultSettings.nbdigits);
		var steps = [];
		for (var precision = 0; precision <= digits; precision++) {
			steps.push(Math.pow(10, -precision));
		}

		container.replaceChildren();
		function addButton(step, sign) {
			var delta = step * sign;
			var label = (sign < 0 ? "−" : "+") + formatStep(step);
			var button = document.createElement("button");
			button.type = "button";
			button.className = "tws-step-button";
			button.dataset.delta = delta.toString();
			button.textContent = label;
			button.title = (sign < 0 ? "Decrease" : "Increase") + " TWS by " + step + " kt";
			button.setAttribute("aria-label", button.title);
			container.appendChild(button);
		}

		steps.forEach(function(step) { addButton(step, -1); });
		steps.slice().reverse().forEach(function(step) { addButton(step, 1); });
	}
	...
	function handleRacesVersion(srvVersion) {
		if (_versions.data.races === undefined || _versions.data.races !== srvVersion) {
			$.ajax({
				...
				cache	: false,
				dataType: "json",
				url		: "https://vro.civis.net/LSV-Dash/api?context=api&context_type=races",
				success	: function(data){
	...
	function handleBoatsVersion(srvVersion) {
		if (_versions.data.boats === undefined || _versions.data.boats !== srvVersion) {
			$.ajax({
				...
				cache	: false,
				dataType: "json",
				url		: "https://vro.civis.net/LSV-Dash/api?context=api&context_type=boats",
				success	: function(data){
					if (srvVersion !== undefined && data.version !== srvVersion) return;
	...
	function retrieveUserDataStored() {
		try {
			...
			if (PolarsStorage.isEmpty()) {
				...
				_current_nbdigits = requestedDigits(urlParams, _defaultSettings.nbdigits);
				_current_twa	= (urlParams["twa"]	  !== undefined)	? parseFloat(urlParams["twa"]).fix(_current_nbdigits) : _defaultSettings.twa;
				// _current_twa	= (urlParams["twa"]	  !== undefined)	? Number.parseFloat(urlParams["twa"]).toFixed(_current_nbdigits) : _defaultSettings.twa;
				_current_tws	= (urlParams["tws"]   !== undefined)	? Number.parseFloat(urlParams["tws"]).toFixed(_current_nbdigits) : _defaultSettings.tws;
				// _current_tws	= (urlParams["tws"]   !== undefined)	? parseFloat(urlParams["tws"]).fix(_current_nbdigits) : _defaultSettings.tws;
        ...
        console.log("_current_nbdigits : " + _current_nbdigits);
			}
			//Otherwise, load appropriate data
				...
				if (raceSettings === undefined) {
					...
					var newRaceDigits = requestedDigits(urlParams, (data.appSettings || {}).nbdigits);
					raceSettings = {
						raceId	: race.id,
						nbdigits : newRaceDigits,
						twa		: (urlParams["twa"] !== undefined) ? parseFloat(urlParams["twa"]).fix(newRaceDigits) : _defaultSettings.twa,
						tws		: (urlParams["tws"] !== undefined) ? Number.parseFloat(urlParams["tws"]).toFixed(newRaceDigits) : _defaultSettings.tws,
						options	: {
        ...
        console.log("_current_nbdigits : " + raceSettings.nbdigits);
				}
				else {
					var storedRaceSettings = raceSettings;
					var storedDigits = storedRaceSettings.nbdigits !== undefined
						? storedRaceSettings.nbdigits
						: (data.appSettings || {}).nbdigits;
					var selectedDigits = requestedDigits(urlParams, storedDigits);
					raceSettings = {
						raceId	: storedRaceSettings.raceId.toString(),
						nbdigits : selectedDigits,
						twa		: (urlParams["twa"] !== undefined) ? parseFloat(urlParams["twa"]).fix(selectedDigits) : parseFloat(storedRaceSettings.twa).fix(selectedDigits),
						tws		: (urlParams["tws"] !== undefined) ? Number.parseFloat(urlParams["tws"]).toFixed(selectedDigits) : Number.parseFloat(storedRaceSettings.tws).toFixed(selectedDigits),
						options	: {
							light	: (urlParams["light"] !== undefined)	? parseBool(urlParams["light"])	: storedRaceSettings.options.light,
							reach	: (urlParams["reach"] !== undefined)	? parseBool(urlParams["reach"])	: storedRaceSettings.options.reach,
							heavy	: (urlParams["heavy"] !== undefined)	? parseBool(urlParams["heavy"])	: storedRaceSettings.options.heavy,
							foil	: (urlParams["foil"]  !== undefined)	? parseBool(urlParams["foil"])	: storedRaceSettings.options.foil,
							hull	: (urlParams["hull"]  !== undefined)	? parseBool(urlParams["hull"])	: storedRaceSettings.options.hull
						}
					};
        console.log("_current_nbdigits : " + raceSettings.nbdigits);
				}
				...
				PolarsStorage.merge({
					appSettings	: {
						...
						nbdigits	: raceSettings.nbdigits
					},
					races : [ raceSettings ]
				});
				...
				_currentBoat	=
					...
					_boats.find(function(item){
						return (item.id === "unknown"); });
				_current_nbdigits	= normalizeDigits(raceSettings.nbdigits, (data.appSettings || {}).nbdigits);
	...
	function buildRacesSelector() {
				...
				arrClosed
					...
					.forEach(function(item, idx){
						...
						$("#race_selection .races_closed").append(
							"<div class='race'>" +
								"<div class='imgbtn' data-race='" + item.id + "' title='" + item.name + "'></div>" +
								item.shortName +
							"</div>");
					});
			}
        ...
        if ($(el).data("race") == "archives") {
				  $(el)
					  .css("background-image", "url(img/" + $(el).data("race") + ".jpg)")
					  .off().click(function(){
						  if ($("#race_selection").is(".disabled")) return;
						  $("#race_selection select").val( $(el).data("race") ).trigger("change");
					  });
        } else {
          var myRace = $(el).data("race").toString();
          myRace = myRace.replace('.', '_');
          myRace = myRace.replace(/^\d{4}_/g, "");

				  $(el)
					  .css("background-image", "url(https://static.virtualregatta.com/offshore/leg/" + myRace + ".jpg)")
					  .off().click(function(){
						  if ($("#race_selection").is(".disabled")) return;
						  $("#race_selection select").val( $(el).data("race") ).trigger("change");
					  });
		...
		try {
			...
			if (raceSettings === undefined) {
				...
				raceSettings = {
					...
					nbdigits		: _defaultSettings.nbdigits,
					twa		: _defaultSettings.twa,
					tws		: _defaultSettings.tws,
					options	: _defaultSettings.options
				};
			}
			raceSettings.nbdigits = normalizeDigits(raceSettings.nbdigits, (data.appSettings || {}).nbdigits);
			PolarsStorage.merge({
				appSettings	: {
					selectedRaceId	: raceSettings.raceId,
					nbdigits	: raceSettings.nbdigits
			...
			_currentBoat	=
				...
				_boats.find(function(item){
					return (item.id === "unknown"); });
			_current_nbdigits	= raceSettings.nbdigits;
			buildWindSpeedButtons();
			_current_twa	= raceSettings.twa;
			_current_tws	= raceSettings.tws;
			_chk_opt_light	= raceSettings.options.light;
			...
			// $("#data_twa .val"	).val (_current_twa					).toFixed(_current_nbdigits);
			// $("#data_twa .val"	).val (_current_twa.toFixed(_current_nbdigits));
		...
		try {
			...
			for (var param in urlParams) {
				...
				newURL += "&" + param + "=" + (
					...
					param === "nbdigits"   ? _current_nbdigits	:
					urlParams[param]
				);
			}
	...
	function plotPolarsChart() {
      ...
      console.log("_current_nbdigits in plotPolarsChart : " + _current_nbdigits);
            ...
            $(".sail.Jib"		).next().next().text(data.sailsSpeeds.Jib		.fix(_current_nbdigits+1, true) +  ( ( (data.sailsSpeeds.Jib*tolerance>_current_boatspeed) && (data.sailsSpeeds.Jib!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.LightJib"	).next().next().text(data.sailsSpeeds.LightJib	.fix(_current_nbdigits+1, true) +  ( ( _chk_opt_light && (data.sailsSpeeds.LightJib*tolerance>_current_boatspeed) && (data.sailsSpeeds.LightJib!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.Staysail"	).next().next().text(data.sailsSpeeds.Staysail	.fix(_current_nbdigits+1, true) +  ( ( _chk_opt_heavy && (data.sailsSpeeds.Staysail*tolerance>_current_boatspeed) && (data.sailsSpeeds.Staysail!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.Code0"		).next().next().text(data.sailsSpeeds.Code0		.fix(_current_nbdigits+1, true) +  ( ( _chk_opt_reach && (data.sailsSpeeds.Code0*tolerance>_current_boatspeed) && (data.sailsSpeeds.Code0!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.Spi"		).next().next().text(data.sailsSpeeds.Spi		.fix(_current_nbdigits+1, true) +  ( ((data.sailsSpeeds.Spi*tolerance>_current_boatspeed) && (data.sailsSpeeds.Spi!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.LightGnk"	).next().next().text(data.sailsSpeeds.LightGnk	.fix(_current_nbdigits+1, true) +  ( ( _chk_opt_light && (data.sailsSpeeds.LightGnk*tolerance>_current_boatspeed) && (data.sailsSpeeds.LightGnk!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
            $(".sail.HeavyGnk"	).next().next().text(data.sailsSpeeds.HeavyGnk	.fix(_current_nbdigits+1, true) +  ( ( _chk_opt_heavy && (data.sailsSpeeds.HeavyGnk*tolerance>_current_boatspeed) && (data.sailsSpeeds.HeavyGnk!=_current_boatspeed)) ? (' (' +_current_boatspeed.fix(_current_nbdigits+1, true) + ')' ) : "")  )  ;
...
//		else				delta = parseInt(delta);
		...
		if (isAbsolute === true)
			_current_twa = (delta).fix(_current_nbdigits);
		else
			_current_twa = (_current_twa + delta).fix(_current_nbdigits);
	...
	function setWindSpeed(delta, isAbsolute) {
		...
		else				delta = Number.parseFloat(delta);

		if (isAbsolute) {
			_current_tws = Number.parseFloat(delta).toFixed(_current_nbdigits);
      // _current_tws = delta.fix(2);
    } else {
			_current_tws = Number.parseFloat(Number.parseFloat(_current_tws) + delta).toFixed(_current_nbdigits);
			// _current_tws = (_current_tws + delta).fix(2);
    }
	...
	function registerEventHandlers() {
		...
		$("#btn_twa_m10"		).off().click (function(){ setTWA(-0.1); });
		$("#btn_twa_p10"		).off().click (function(){ setTWA(+0.1); });
		...
		$("#tws_step_buttons").off("click", ".tws-step-button").on("click", ".tws-step-button", function(){
			setWindSpeed(Number.parseFloat(this.dataset.delta));
		});
