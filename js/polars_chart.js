...
var PolarsChart = (function(){
	var _chartRadius		= 300;
	var _leftOffset			= 60.5;
	var _topOffset			= 40.5;//20.5;
	var _chartScale			= {
		graduationValue	: 5,
		nbGraduations	: 5,
		max				: 25
	...
	function refreshLayer(canvasId) {
		// Reset the drawing buffer while preserving the 425x650 logical chart.
		// CSS scales the canvas responsively; changing its intrinsic width here
		// would also change the coordinate system used by every plotting routine.
	...
	function getDataArray(twa, tws, boatType, options) {
		...
		if (recalcVMGs) {
			...
			// for (var i = 0; i <= 180; i = (i + 0.001).fix(3) ) {
    ...
    // if background = black
		...
		// _chartScale.nbGraduations = Math.ceil( (maxSpeed + 0) / _chartScale.graduationValue );
		_chartScale.nbGraduations = Math.ceil( (maxSpeed + 0.01) / _chartScale.graduationValue );
		_chartScale.max =
			_chartScale.graduationValue * _chartScale.nbGraduations;

		//Arcs
		c.textAlign = "right";
		c.fillText("0 kts", -2, 0);
	...
	function plotMaxValues(bRefreshLayer) {
		...
		c.strokeStyle	= "green";
		c.beginPath();
		radial(_currentResultset.max.twa, 25, false, c, "°", [3, 5]);
		c.stroke();
	}
	...
	function getMousePos(layer, evt) {
		...
		return {
			x : (evt.clientX - rect.left) * layer.canvas.width / rect.width,
			y : (evt.clientY - rect.top) * layer.canvas.height / rect.height
		};
	}
