...
var PolarsReader = (function() {
	...
	var _polars			= {};
	var _typesRetrieved = {};
  var _current_nbdigits = 5;

	function getSpeeds(twa, tws, boatType, options) {
    // console.log("twa in getSpeeds: " + twa);
    // console.log("options in getSpeeds: " + options);
		...
		if (_typesRetrieved[boatType] === undefined) {
			...
			$.ajax({
				...
				cache	: false,
				dataType: "json",
				url		: "https://vro.civis.net/LSV-Dash/api?context=api&context_type=polars&boatType=" + boatType,
    ...
    // console.log("best in getSpeeds: " + theoreticalSpeed(twa, tws, options));
    // console.log("all in getSpeeds: " + allSailsSpeeds(twa, tws, options));
    let PolarUpdated = new Date(_polars._updatedAt);
    $("#data_polar_updated"	).text(PolarUpdated.toLocaleDateString() + " " + PolarUpdated.toLocaleTimeString());
    // $("#data_polar_updated"	).text(_polars._updatedAt);
    ...
    // console.log("twa in theoreticalSpeed: " + twa);
...
//		return {
//      speed   : totalSpeed.fix(_current_nbdigits, true),
//			speed		: roundTo(totalSpeed, _current_nbdigits),
//			vmg			: roundTo(totalSpeed * Math.cos(twa * Math.PI/180), _current_nbdigits),
//			sail		: speed.sail,
//			foilFactor	: roundTo(foilFactor, _current_nbdigits),
//			foilRate	: roundTo(foilRate, _current_nbdigits)
//		};
		return {
			speed		: roundTo(totalSpeed, _current_nbdigits),
			vmg			: roundTo(totalSpeed * Math.cos(twa * Math.PI/180), _current_nbdigits),
    ...
    // console.log("twa in allSailsSpeeds: " + twa);
		var globalFactor= _polars.globalSpeedRatio;
		var foilFactor	= foilingFactor(options, tws, twa, _polars.foil);
		var hullFactor	= hullingFactor(options, _polars.hull);
		var twsLookup	= fractionStep(tws, _polars.tws);
    ...
    // console.log("POLAR: " + JSON.stringify(_polars.sail));
		$.map(speeds, function(val, key){
			// speeds[key] = roundTo(val * foilFactor * hullFactor * globalFactor, 3);
			speeds[key] = roundTo(val * foilFactor * hullFactor * globalFactor, _current_nbdigits);
		});

    // console.log("allSailsSpeeds : " + JSON.stringify(speeds));
		return speeds;
	}
